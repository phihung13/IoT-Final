...
async def get_graph_data():
    try:
        ...
        for field, collection in fields_and_collections:
            data_points = list(collection.find({}, sort=[('_id', pymongo.DESCENDING)], limit=num_data_points))
            x = [datetime.strptime(str(dp['time']), "%Y-%m-%d %H:%M:%S") for dp in data_points]
            y = [dp[field] for dp in data_points]
            ...
            result[field] = {
                "data": {"time": [str(ts) for ts in x[::-1]], "values": y[::-1]},
                "chart": image_base64,
            }

            html += f'<img src="data:image/png;base64,{image_base64}" alt="Field {field} chart"><br>'
        ...
        raise HTTPException(status_code=500, detail=f"Internal Server Error: {str(e)}")
###########################################################################################
def read_mqtt_status():
    with open("status.txt", "r") as mqtt_status:
        return mqtt_status.read()
###########################################################################################
